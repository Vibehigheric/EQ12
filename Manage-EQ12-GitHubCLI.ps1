# EQ12 GitHub CLI Manager - Task Wrapper
...
switch ($Action.ToLower()) {
    'status' {
        Write-EQ12Status "Checking GitHub CLI status..." "📊"
    ...
    'download' {
        Write-EQ12Status "Downloading GitHub CLI installer..." "⬇️"
        ...
        try {
            ...
            if (Test-Path $downloadPath) {
                ...
                Write-Host "✅ Download completed successfully ($([math]::Round($fileSize, 1)) MB)" -ForegroundColor Green
            }
            else {
    ...
    'test' {
        Write-EQ12Status "Running GitHub CLI functionality tests..." "🧪"
    ...
    'configure' {
        Write-EQ12Status "Configuring GitHub CLI for EQ12..." "⚙️"

        $status = Get-GitHubCLIStatus

        if (-not $status.installed) {
            Write-Host "❌ GitHub CLI not installed. Run with -Action Install first." -ForegroundColor Red
            exit 1
        }
        ...
        if (-not (Test-Path $tokenFile)) {
            Write-Host "❌ GitHub token file not found: $tokenFile" -ForegroundColor Red
    ...
    'install' {
        Write-EQ12Status "Starting GitHub CLI installation process..." "🚀"
        ...
        if (-not (Test-Path $installerPath)) {
            Write-Host "❌ Installer MSI not found. Downloading..." -ForegroundColor Yellow

            # Download first
            & $MyInvocation.MyCommand.Path -Action Download

            if ($LASTEXITCODE -ne 0) {
                Write-Host "❌ Download failed. Cannot proceed with installation." -ForegroundColor Red
                exit 1
            }
        }
...
Write-EQ12Status "GitHub CLI Manager action completed: $Action" "✅"
