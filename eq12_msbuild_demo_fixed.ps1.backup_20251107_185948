...
function Test-VBNetProject($projectPath, $projectName) {
    ...
    if ($projectContent -match "EQ12.*Target") {
        ...
        # Count targets
        $targetMatches = [regex]::Matches($projectContent, '<Target Name="EQ12[^"]*"')
    ...
    if (Test-Path $logDir) {
        ...
        foreach ($log in $logFiles) {
            ...
            if (Test-Path $logPath) {
                ...
                $logDisplay = "$($log.Name) ($size bytes, modified: $modified)"
                Write-Host "   ✅ $logDisplay" -ForegroundColor Green
...
function Test-BuildProcess($projectPath) {
    ...
    try {
        ...
        if ($buildSuccess) {
            ...
            $errorLines = $buildOutput | Select-Object -Last 5
