...
class AdvancedParlayGenerator:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        logger.info(
            f"Initialized AdvancedParlayGenerator for {
    ...
    def generate_parlay_slips(
            self, target_legs: list[int]) -> dict[int, list[ParlaySlip]]:
        ...
        filtered_bets = [bet for bet in all_bets if -300 <=
                         bet.odds <= 300 and bet.implied_probability > 0.2]

        # Limit SGPs to best combinations (highest probability)
        sgps.sort(
            key=lambda x: sum(
                bet.implied_probability for bet in x.bets),
            reverse=True)
        filtered_sgps = sgps[:50]  # Keep top 50 SGPs

        # Combine individual bets and SGPs as potential legs (limited set)
        # Limit total legs to manageable number
        all_legs = filtered_bets[:100] + filtered_sgps

        logger.info(
            f"Using {
                len(all_legs)} potential legs ({
                len(filtered_bets)} bets + {
                len(filtered_sgps)} SGPs)")
        ...
        for leg_count in target_legs:
            ...
            while len(slips) < 10 and attempts < max_attempts:
                ...
                for leg in all_legs:
                    ...
                    if isinstance(leg, Bet):
                        # For longer parlays, allow multiple markets per game but avoid
                        # direct conflicts
    ...
    def format_parlay_output(self, parlay_slips: dict[int, list[ParlaySlip]]) -> str:
        ...
        output.append(
            "💡 DISCLAIMER: These parlays are for entertainment purposes only.")
        output.append("   Please gamble responsibly and within your means.")
        output.append(
            "   Same Game Parlays (SGPs) have correlated outcomes - bet accordingly.")
    ...
    def save_analysis(self, parlay_slips: dict[int, list[ParlaySlip]]) -> str:
        ...
        # Save to file
        filename = f"advanced_parlay_analysis_{
