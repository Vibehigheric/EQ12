...
def american_to_decimal(am_odd: float) -> float:
    ...
    odd = 1 - 100 * 1.0 / am_odd if am_odd < 0 else 1 + am_odd * 1.0 / 100
...
def find_most_balanced(
    side_1_outcomes: list[dict], side_2_outcomes: list[dict], american_odds_format: bool
) -> tuple[dict, dict]:
    ...
    for point in side_1_by_point:
        ...
        if american_odds_format:
            diffs[point] = abs(
                american_to_decimal(side_1["price"]) - american_to_decimal(side_2["price"])
            )
        else:
