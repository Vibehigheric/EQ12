...
def find_most_balanced_totals(
    outcomes: list[dict], american_odds_format: bool
) -> tuple[dict, dict]:
...
def find_most_balanced_team_totals(
    outcomes: list[dict], home_team: str, away_team: str, american_odds_format: bool
    ...
    If prices are in American odds format, set american_odds_format to True
    """
    home_team_overs = list(
        filter(lambda x: x["name"] == "Over" and x["description"] == home_team, outcomes)
    )
    home_team_unders = list(
        filter(lambda x: x["name"] == "Under" and x["description"] == home_team, outcomes)
    )
    home_team_most_balanced = find_most_balanced(
        home_team_overs, home_team_unders, american_odds_format
    )

    away_team_overs = list(
        filter(lambda x: x["name"] == "Over" and x["description"] == away_team, outcomes)
    )
    away_team_unders = list(
        filter(lambda x: x["name"] == "Under" and x["description"] == away_team, outcomes)
    )
    away_team_most_balanced = find_most_balanced(
        away_team_overs, away_team_unders, american_odds_format
    )
...
if __name__ == "__main__":
    ...
    alternate_totals = next(filter(lambda x: x["key"] == "alternate_totals", bookmaker["markets"]))
    if not alternate_totals:
        print("Alternate totals market not found from first bookmaker")
        print()

    else:
        most_balanced_totals = find_most_balanced_totals(
            alternate_totals["outcomes"], american_odds_format
        )
        print()
    ...
    alternate_team_totals = next(
        filter(lambda x: x["key"] == "alternate_team_totals", bookmaker["markets"])
    )
    if not alternate_team_totals:
        print("Alternate team totals market not found from first bookmaker")

    else:
        home_team_most_balanced, away_team_most_balanced = find_most_balanced_team_totals(
            alternate_team_totals["outcomes"],
            response["home_team"],
            response["away_team"],
