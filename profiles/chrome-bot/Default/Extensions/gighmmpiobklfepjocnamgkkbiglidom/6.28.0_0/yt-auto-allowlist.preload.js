...
function handleVideoPlaying(event) {
    ...
    if (video[alreadyLoggedSymbol] === video.src) {
        return;
    }
    const playerContainer = video.closest(".ad-showing");
    if (playerContainer) {
        const skippable = !!playerContainer.querySelector(".ytp-skip-ad-button");
        video[alreadyLoggedSymbol] = video.src;
        void recordEvent(adShownEvent, {
            category: skippable ? "skippable" : "non-skippable",
            bc: video.duration,
        });
    }
}
function addPlayListener(video) {
...
function handleMutations(mutations) {
    for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
            ...
            const elements = [
                node,
                ...Array.from(node.querySelectorAll([videoSelector, adWallSelector, toastSelector].join(","))),
            ];
            for (const elem of elements) {
                if (elem.tagName === "VIDEO") {
                    addPlayListener(elem);
                }
                else if (elem.tagName === adWallSelector.toUpperCase() &&
                    !wallDetected) {
