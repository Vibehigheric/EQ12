...
var CAMELIZER = new Object({
  ...
  config: function() {
    ...
    if (key == "default_chart_dims")
    {
      switch (CAMELIZER.setting("window_size"))
      {
        case "small":
          return(this.configs["default_chart_dims_small"]);
        //case "large":
        //  return(this.configs["default_chart_dims_large"]);
        case "default":
        default:
          return(this.configs["default_chart_dims"]);
      }
    }
...
function executeScript(tabId, script, runAt, allFrames) {
  ...
  return new Promise((resolve, reject) => {
    try {
      chrome.scripting.executeScript(
        ...
        (response) => {
          resolve(response.map(i => i.result))
        },
      );
    } catch (e) {
      reject(e);
...
function announce(callback)
{
  ...
  querying.then(function(tab) {
    ...
    var executing = executeScript(tab.id, "announce.js", "document_idle", false);
    
    executing.then(function(response) {
      callback(response);
    }, function(response) {
      dmsg("ERROR ANNOUNCING");
      dmsg(response);
    });
  }, function(response) {
    dmsg("ERROR ANNOUNCING");
...
function get_asins(callback)
{
  ...
  querying.then(function(tab) {
    ...
    var executing = executeScript(tab.id, "get_all_anchors.js");
...
function get_asin(callback)
{
  ...
  querying.then(function(tab) {
    ...
    var executing = executeScript(tab.id, "get_asin.js");
...
function set_firstrun_options(callback)
{
  browser.tabs.query({active: true, currentWindow: true})
    .then(tab => executeScript(tab[0].id, "set_firstrun_options.js"))
      .then(response => callback(response[0])
    )
    .catch(resp => dmsg("error setting options", resp));
}
...
function set_chart_height(h)
{
  ...
  {
    case "small":
      parent_height = 460 - 10;
      break;
    //case "medium":
   //   parent_height = 580 - 10;
    //  break;
   // case "large":
    //  parent_height = 700 - 10;
    //  break;
    case "default":
    default:
      parent_height = 580 - 10;
...
function is_amazon_blacklist(url)
{
...
{
  return("https://camelcamelcamel.com/camelizer/first_run?browser=chrome&version=3.0.16&utm_campaign=camelizer&utm_medium=extension&utm_source=chrome&utm_content=first_run");
}

function last_run_url()
{
  return("https://camelcamelcamel.com/camelizer/last_run?browser=chrome&version=3.0.16&utm_campaign=camelizer&utm_medium=extension&utm_source=chrome&utm_content=last_run");
}
...
function parse_amazon_url_and_update_ui(page_asin)
{
...
function show_loading_error_message(title, message) {
  set_error_overlay_text(title, message);
  $("#pricehistory_tab_btn").addClass("disabled");
  $("#chartmenu").addClass("disabled");
...
function finish_parse_amazon_url_and_update_ui()
{
  ...
  $.get(api_url, secret_keys, function(){}).always(function(data, status, xhr) {
    ...
    if (data.p)
    {
      show_loading_error_message(browser.i18n.getMessage("parent_error_title"), browser.i18n.getMessage("parent_error_msg"));
    ...
    update_user_info_from_response(data);
...
function process_asins_for_pageproducts(resp)
{
  ...
  if (asins.length == 0) {
    // jquery doesn't send empty arrays at all and our backend freaks out
    data.asins = [""];
  }
...
function update_user_info()
{
  ...
  if (CAMELIZER.ephemeral("logged_in"))
  {
    ...
    $("#youraccountbtn").show();
    $("#signedinas").show();
    $("#user_tab_btn").removeClass("alert");
...
function update_user_info_from_response(data) {
  dmsg("Updating user info from response");
  dmsg(data);

  if (data.a || data.logged_in)
  {
...
{
  var window_size = CAMELIZER.setting("window_size") || "default";
  $("html, body, #container").removeClass("small_window medium_window large_window").addClass(window_size + "_window");
}
browser.pageAction = chrome.action; // missing polyfill
