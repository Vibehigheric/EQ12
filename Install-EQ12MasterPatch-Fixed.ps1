...
# Check if meta_search directory exists, if not create it
if (!(Test-Path $MetaSearchDir)) {
    Write-Host "Creating eq12_meta_search directory..." -ForegroundColor Yellow
    New-Item -ItemType Directory -Path $MetaSearchDir -Force | Out-Null
}
...
try {
    ...
    Write-Host "⚠️ Playwright install failed - may need manual setup" -ForegroundColor Yellow
}
...
if (Test-Path "hlayer_wrapper.py") {
    try {
        python hlayer_wrapper.py --init-config | Out-Null
        Write-Host "✅ HumanLayer config initialized" -ForegroundColor Green
    } catch {
        Write-Host "⚠️ HumanLayer config failed - may need manual setup" -ForegroundColor Yellow
    }
} else {
    Write-Host "⚠️ hlayer_wrapper.py not found" -ForegroundColor Yellow
}
...
# Quick test if dashboard.py exists and is valid
if (Test-Path "dashboard.py") {
    Write-Host "✅ Dashboard file found" -ForegroundColor Green
} else {
    Write-Host "⚠️ dashboard.py not found - may need manual setup" -ForegroundColor Yellow
}
