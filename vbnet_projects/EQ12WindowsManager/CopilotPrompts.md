# EQ12 VB.NET Copilot Prompts - EQ12WindowsManager
...
1. **Create robust VB.NET class with proper error handling and XML documentation**

   ```vb
   ' Create robust VB.NET class with proper error handling and XML documentation
   ```

2. **Implement async method with cancellation token and ConfigureAwait(false)**

   ```vb
   ' Implement async method with cancellation token and ConfigureAwait(false)
   ```

3. **Add comprehensive logging with Microsoft.Extensions.Logging**

   ```vb
   ' Add comprehensive logging with Microsoft.Extensions.Logging
   ```

4. **Create dependency injection setup with interface abstractions**

   ```vb
   ' Create dependency injection setup with interface abstractions
   ```

5. **Implement SOLID principles in VB.NET with proper abstraction layers**

   ```vb
   ' Implement SOLID principles in VB.NET with proper abstraction layers
   ```

6. **Design responsive Windows Forms with modern UI patterns**

   ```vb
   ' Design responsive Windows Forms with modern UI patterns
   ```

7. **Implement data binding with INotifyPropertyChanged interface**

   ```vb
   ' Implement data binding with INotifyPropertyChanged interface
   ```

8. **Create custom controls with proper event handling**

   ```vb
   ' Create custom controls with proper event handling
   ```

9. **Add form validation with ErrorProvider and user feedback**
