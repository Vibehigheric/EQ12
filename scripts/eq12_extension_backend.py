...
if FIREFOX_INTEGRATION_AVAILABLE and register_firefox_extension_endpoints:
    ...
    logger.warning(
        "⚠️  Firefox Extension integration not available - endpoints not registered")

# Register CFB Backend API Endpoints
try:
    from eq12_cfb_backend import register_cfb_endpoints

    register_cfb_endpoints(app)
    logger.info("✅ CFB Backend API endpoints registered successfully")
except ImportError as e:
    logger.warning(f"⚠️  CFB Backend integration not available: {e}")
    # Create fallback CFB endpoints inline

    @app.get("/api/cfb/health")
    async def cfb_health_fallback():
        """Fallback CFB health endpoint"""
        return {
            "status": "fallback",
            "message": "CFB Backend module not available",
            "timestamp": datetime.now(UTC).isoformat(),
        }

    logger.info("✅ CFB Backend fallback endpoints registered")
except Exception as e:
    logger.error(f"❌ Failed to register CFB Backend endpoints: {e}")
...
def calculate_consensus_fair_value(all_prices: list[float]) -> float:
    ...
    if len(implied_probs) > 3:
        ...
        std_dev = (sum((x - mean_prob) ** 2 for x in implied_probs) /
                   len(implied_probs)) ** 0.5
...
def decide_and_place_bet(
    ...
    if edge < bankroll_mgr.min_edge:
        logger.debug(
    f"Edge {
        edge:.3f} below threshold {
            bankroll_mgr.min_edge} for {selection}")
        return None
    ...
    logger.info(
        f"✅ Edge bet identified: {selection} - Edge: {
    edge:.1%}, Kelly: {
        kelly_fraction:.1%}, Size: ${
            bet_size:.2f}"
    )
    return edge_bet
...
class NFLBettingEngine:
    ...
    def nfl_decide_and_place(
        ...
        if adjusted_edge < self.min_edge_threshold:
            logger.debug(
                f"NFL edge {
    adjusted_edge:.3f} below threshold for {
        game.away_team}@{
            game.home_team}"
            )
            return None
        ...
        logger.info(
            f"🏈 NFL EDGE: {selection} | Edge: {
    adjusted_edge:.1%} | Kelly: {
        final_kelly:.1%} | Size: ${
            bet_size:.2f} | {confidence}"
        )
        return edge_bet
    ...
    def _validate_nfl_constraints(self, game: NFLGame, market_type: str) -> bool:
        ...
        if len(week_bets) >= self.max_bets_per_week:
            logger.debug(
                f"Weekly bet limit reached: {len(week_bets)}/{self.max_bets_per_week}")
            return False
        ...
        for team in [game.home_team, game.away_team]:
            ...
            if current_exposure >= self.max_exposure_per_team:
                logger.debug(
    f"Team exposure limit reached for {team}: {
        current_exposure:.1%}")
                return False
...
class NFLGameLinesEngine:
    ...
    def normalize_implied_probs(
        self, implied_probs: dict[str, float]) -> dict[str, float]:
    ...
    def compute_edge_total(
    self,
    model_p_over: float,
     implied_p_over_norm: float) -> float:
    ...
    def decide_and_place_nfl_game_lines(
        ...
        # Check Home ML edge
        edge_home = self.compute_edge_moneyline(
    prediction.home_win_prob, imp_ml_norm["home"])
        ...
        # Check Away ML edge
        edge_away = self.compute_edge_moneyline(
    prediction.away_win_prob, imp_ml_norm["away"])
        ...
        # Check Over edge
        edge_over = self.compute_edge_total(
    prediction.over_prob, imp_total_norm["over"])
        ...
        # Check Under edge
        edge_under = self.compute_edge_total(
    prediction.under_prob, imp_total_norm["under"])
...
class NFLTDScorerEngine:
    ...
    def extract_td_prop_features(
        self, game: NFLGame, player: NFLPlayer) -> dict[str, float]:
        ...
        features = {
            ...
            # RBs score more TDs
            "position_weight": (1.0 if player.position == "RB" else 0.8),
    ...
    def predict_td_probability(self, features: dict[str, float]) -> float:
        ...
        # Game environment
        base_prob *= features["game_total_projection"] / \
            45.0  # Scale by expected scoring
...
class NFLPassingPropsEngine:
    ...
    def extract_passing_yards_features(
        ...
        return {
            ...
            # 0-1 scale
            "opponent_def_rank_scaled": (33 - qb_props.opponent_pass_def_rank) / 32,
    ...
    def extract_passing_td_features(
        ...
        return {
            ...
            # Team red zone TD rate (would pull from stats)
    ...
    def predict_passing_yards_prob_over(
        self, features: dict[str, float], line: float) -> float:
    ...
    def predict_passing_td_prob_over(
        self, features: dict[str, float], line: float) -> float:
        ...
        # Apply contextual adjustments
        projected_tds *= features["team_total_projection"] / \
            24.0  # Scale by expected scoring
    ...
    def passing_prop_decide_and_place(self, game: NFLGame) -> list[EdgeBet]:
        ...
        for qb_props in [home_qb, away_qb]:
            ...
            prob_over_yards = self.predict_passing_yards_prob_over(
                yards_features, yards_line)
...
class NFLReceivingProps:
    ...
    # Matchup Context
    # Yards allowed to WR/TE/RB
    opponent_def_vs_position: dict[str, float] | None = None
...
class NFLReceivingPropsEngine:
    ...
    def pick_receiving_prop_candidates(self, game: NFLGame) -> list[NFLReceivingProps]:
        ...
        return [c for c in candidates if c.target_share >=
            0.10 and c.avg_receptions >= 2.5]
    ...
    def predict_receiving_yards_over(
        self, features: dict[str, float], line: float) -> float:
        ...
        # Game environment
        projected_yards *= features["team_passing_volume"] / \
            32.0  # Scale by pass volume
    ...
    def receiving_prop_decide_and_place(self, game: NFLGame) -> list[EdgeBet]:
        ...
        for player in candidates:
            ...
            prob_over_yards = self.predict_receiving_yards_over(
                yards_features, yards_line)
            ...
            if edge_over >= self.EDGE_THRESHOLD_PROP:
                ...
                if bet_size >= self.MIN_BET:
                    edge_bet = EdgeBet(
                        ...
                        selection=f"{
    player.player_name} Over {yards_line} Receiving Yards",
                        book="Combined",
                        odds=odds_over,
                        implied_prob=implied_prob,
                        fair_prob=prob_over_yards,
...
class NFLRushingPropsEngine:
    ...
    def extract_rushing_yards_features(
        ...
        return {
            ...
            # QBs less consistent
            "position_factor": (1.0 if player.position == "RB" else 0.6),
            "team_rush_attempts": 28.0,  # Expected team rush attempts
        }

    def predict_rushing_yards_over(
        self, features: dict[str, float], line: float) -> float:
    ...
    def rushing_prop_decide_and_place(self, game: NFLGame) -> list[EdgeBet]:
        ...
        for player in candidates:
            ...
            prob_over_yards = self.predict_rushing_yards_over(
                yards_features, yards_line)
            ...
            if edge_over >= self.EDGE_THRESHOLD_PROP:
                ...
                if bet_size >= self.MIN_BET:
                    edge_bet = EdgeBet(
                        ...
                        selection=f"{
    player.player_name} Over {yards_line} Rushing Yards",
                        book="Combined",
                        odds=odds_over,
                        implied_prob=implied_prob,
                        fair_prob=prob_over_yards,
...
class NFLHalvesEngine:
    ...
    def extract_half_features_pre(self, game: NFLGame, half: str) -> dict[str, float]:
        ...
        return {
            ...
            # Home teams start stronger
            "home_field_hal": (1.5 if half == "first" else 1.0),
    ...
    def predict_half_over_prob(self, predicted_total: float,
                               market_line: float) -> float:
...
class NCAAFBettingEngine:
    ...
    def __init__(self, bankroll_mgr: GPT5BankrollManager):
        ...
        # NCAAF edge thresholds (higher due to market inefficiencies but lower
        # confidence)
    ...
    def ncaaf_decide_and_place(
        ...
        implied_prob = self._calculate_ncaaf_implied_prob(
            market_odds, market_type, game)
        raw_edge = fair_prob - implied_prob

        # Step 3: Apply college-specific edge adjustments
        adjusted_edge = self._adjust_edge_for_ncaaf_factors(
    ...
    adjusted_edge:.3f} below threshold for {
        ...
        kelly_multiplier = self._apply_ncaaf_kelly_adjustments(
            kelly_multiplier, game, prediction)
        ...
        bet_size = self.bankroll_mgr.calculate_bet_size(final_kelly)
        confidence = self._classify_ncaaf_confidence(
            adjusted_edge, final_kelly, game, prediction)
    ...
    def _calculate_ncaaf_implied_prob(
        ...
        if market_type == "spread":
            ...
            # This is a simplified conversion - in practice you'd get the spread from
            # odds API
    ...
    def _validate_ncaaf_constraints(self, game: NCAAFGame, market_type: str) -> bool:
        ...
        if len(week_bets) >= self.max_bets_per_week:
            logger.debug(
                f"NCAAF weekly bet limit reached: {
    len(week_bets)}/{
        self.max_bets_per_week}"
            )
            return False
        ...
        for team in [game.home_team, game.away_team]:
            ...
            if current_exposure >= self.max_exposure_per_team:
                logger.debug(
                    f"NCAAF team exposure limit reached for {team}: {
    current_exposure:.1%}"
                )
                return False
        ...
        for conf in [game.home_conference, game.away_conference]:
            if conf != "FBS Independent":
                ...
                if current_exposure >= self.max_exposure_per_conference:
                    logger.debug(
                        f"NCAAF conference exposure limit reached for {conf}: {
    current_exposure:.1%}"
                    )
                    return False
...
def _calculate_ncaaf_team_strength(game: NCAAFGame, is_home: bool) -> float:
    ...
    experience_adjustment = (returning_starters - 11) * \
                             0.2  # Each starter worth 0.2 pts
...
class MLBGame:
    ...
    wind_direction: str | None = None  # "out to R", "in from CF", etc.
    precipitation_chance: float | None = 0.0

    # Game Context
    day_night: str = "night"  # "day" or "night"
    game_type: str = "regular"  # "regular", "playof", "wildcard"
...
class MLBBettingEngine:
    ...
    def mlb_decide_and_place(
        ...
        try:
            ...
            max_bet = self.bankroll.current_bankroll * \
                self.mlb_config["max_bet_fraction"]
            ...
            confidence = self._classify_mlb_confidence(
                adjusted_edge, final_kelly, game, prediction)
            selection = self._format_mlb_selection(
    market_type, game, market_odds, prediction)
    ...
    def _classify_mlb_confidence(
        ...
        if game.wind_speed and game.wind_speed > 20:
            if confidence == "low":
                confidence = "medium"
    ...
    def _track_mlb_bet(self, bet: EdgeBet, game: MLBGame,
                       prediction: MLBMarketPrediction):
        ...
        logger.info(
            f"Prediction: H:{
    prediction.home_win_prob:.1%} A:{
        prediction.away_win_prob:.1%}"
        )
...
def generate_mlb_prediction(game: MLBGame) -> MLBMarketPrediction:
    ...
    home_run_advantage = home_predicted_runs / \
        (home_predicted_runs + away_predicted_runs)
    ...
    f5_run_advantage = (
        f5_home_runs / (f5_home_runs + f5_away_runs) if (f5_home_runs +
                        f5_away_runs) > 0 else 0.5
    )
    ...
    game_volatility = calculate_game_volatility(
    game, home_pitcher_quality, away_pitcher_quality)
...
def calculate_game_volatility(
    game: MLBGame,
    home_pitcher_q: float,
     away_pitcher_q: float) -> float:
...
class NBABettingEngine:
    ...
    def nba_decide_and_place(
        ...
        adjusted_edge = self._adjust_edge_for_nba_factors(
    ...
    adjusted_edge:.3f} below threshold for {
        ...
        bet_size = self.bankroll_mgr.calculate_bet_size(final_kelly)
        confidence = self._classify_nba_confidence(
    adjusted_edge, final_kelly, game, prediction)
        selection = self._format_nba_selection(
    market_type, game, market_odds, prediction)
    ...
    def _adjust_edge_for_nba_factors(
        ...
        total_load_management = len(game.home_load_management) + \
                                    len(game.away_load_management)
    ...
    def _validate_nba_constraints(self, game: NBAGame, market_type: str) -> bool:
        ...
        if home_exposure > self.max_exposure_per_team or away_exposure > self.max_exposure_per_team:
            logger.debug(
                f"Team exposure limit reached: {game.home_team} = (
                    {home_exposure:.1%}, {game.away_team}={away_exposure:.1%}"
                )
            )
            return False
...
async def generate_nfl_parlay(
    ...
    try:
        ...
        rationale = (
            f"NFL Edge Analysis: {len(selected_bets)} high-conviction bets with {avg_edge:.1%} average edge. "
        )
...
async def generate_mock_ncaaf_parlay(
    ...
    return ParlayResponse(
        ...
        rationale = (
            f"Mock college football parlay with {min_edge:.1%} edge targeting market inefficiencies in NCAAF games.",
        )
...
async def generate_mock_mlb_parlay(
    ...
    mock_legs = []
    for i in range(size):
        home_team, home_pitcher, away_team, away_pitcher = random.choice(mlb_teams)
...
async def generate_nba_parlay(
    ...
    try:
        ...
        parlay_implied_prob = 1.0 / combined_odds
        ...
        efficiency_games = sum(1 for bet in selected_bets if "efficiency" in bet.selection.lower())
...
async def generate_ncaab_parlay(
    ...
    tournament_round: str = Query(
        "round_1",
        description = (
            "Tournament round: first_four, round_1, round_2, sweet_16, elite_8, final_four, championship",
        )
    ),
    market_focus: str = Query(
        "mixed",
    ...
    try:
        ...
        logger.info(
            f"{tournament_emoji} Generating NCAAB parlay: size = (
                {size}, mode={season_mode}, round={tournament_round}"
            )
        )
...
def calculate_expected_value(combined_odds: float, true_prob: float, bet_amount: float) -> float:
    ...
    if combined_odds > 0:
        ...
        decimal_odds = (100 / abs(combined_odds)) + 1
