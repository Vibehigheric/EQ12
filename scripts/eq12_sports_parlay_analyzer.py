...
class SportsParlay:
    ...
    def _analyze_moneyline(self, market: dict, game: dict) -> list[dict]:
        ...
        for outcome in market["outcomes"]:
            ...
            implied_prob = 100 / \
                (price + 100) if price > 0 else abs(price) / (abs(price) + 100)

            # Look for potential value (this is simplified - real analysis would use
            # more data)
                ...
                bets.append(
                    {
                        ...
                        "implied_probability": round(
                            implied_prob * 100,
                            1),
                        "reasoning": f"Reasonable odds at {price} for {
                            outcome['name']}",
                    })
                    ...
                    bets.append(
                        {
                            ...
                            "selection": f"{
                                ...
                                point:+.1f}",
                            "odds": price,
                            "reasoning": f"Close spread of {
                                point:+.1f} suggests competitive game",
                        })
        ...
        for i in range(len(high_value_games)):
            for j in range(i + 1, len(high_value_games)):
                ...
                if game1["best_bets"] and game2["best_bets"]:
                    ...
                    parlays.append({"legs": [{"game": game1["teams"],
    ...
    def run_analysis(self) -> dict[str, Any]:
        ...
        for sport_name, sport_key in self.sports.items():
            if sport_key not in sport_keys:
                logging.warning(
                    f"Sport {sport_key} not available, trying alternative...")
...
def main():
    ...
    parser.add_argument(
        "--save-only",
        action="store_true",
        help="Save to logs without printing")
