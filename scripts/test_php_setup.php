 ...
 * Tests Intelephense, Xdebug, and XAMPP integration
 */
class EQ12SetupValidator 
{
    ...
    private function checkPHPVersion(): void 
    {
        ...
        echo "{$status} PHP Version: {$version} (Required: >= 8.2)\n";
    }
    ...
    private function checkXdebugExtension(): void 
    {
        ...
        $this->results['xdebug'] = [
            ...
            'mode' => $xdebugMode,
            'status' => $xdebugLoaded ? 'PASS' : 'FAIL'
        ];
        
        $status = $xdebugLoaded ? '✅' : '❌';
        echo "{$status} Xdebug Extension: " . ($xdebugLoaded ? "Loaded (v{$xdebugVersion}, mode: {$xdebugMode})" : "Not loaded") . "\n";
    }
    ...
    /**
     * Test PHP 8.2+ type hinting features
     */
    private function testTypeHinting(): void 
    {
        try {
            // Test union types
            $value = $this->processUnionType("test");
            ...
            echo "✅ Type Hinting: All PHP 8.2+ features working\n";
     ...
     */
    private function processUnionType(string|int|null $value): string 
    {
     ...
     * @param array{home: int, away: int, total: float} $oddsData
     */
    ...
    private function testArrayOperations(): void 
    {
        try {
            ...
            // Test array_filter with arrow functions  
            $filtered = array_filter($combined, fn($x) => $x % 2 === 0);
            
            // Test array_map with arrow functions
            $mapped = array_map(fn($x) => $x * 2, $filtered);
            
            $this->results['array_operations'] = [
                'spread_operator' => count($combined) === 6,
                'arrow_functions' => count($filtered) === 3,
                'functional_style' => count($mapped) === 3,
                'status' => 'PASS'
            ];
            
            echo "✅ Array Operations: Modern PHP features working\n";
    ...
    public function getSummary(): array 
    {
        ...
        $passedTests = count(array_filter($this->results, fn($result) => $result['status'] === 'PASS'));
        $warnTests = count(array_filter($this->results, fn($result) => $result['status'] === 'WARN'));
        $failedTests = count(array_filter($this->results, fn($result) => $result['status'] === 'FAIL'));
...
if (__FILE__ === realpath($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    $validator = new EQ12SetupValidator("EQ12 PHP Development Environment");
