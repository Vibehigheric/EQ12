...
function Show-MonitoringDashboard {
    param([string]$IP, [int]$Interval)
    
    Write-Host " EQ12 Pi TPU Monitoring Dashboard" -ForegroundColor Green
    Write-Host "=" * 60 -ForegroundColor Cyan
    Write-Host "Pi IP: $IP | Refresh: ${Interval}s | Ctrl+C to exit" -ForegroundColor Yellow
    Write-Host ""
    ...
    while ($true) {
        try {
            ...
            Write-Host " EQ12 Pi TPU Monitoring Dashboard" -ForegroundColor Green
            Write-Host "=" * 60 -ForegroundColor Cyan
            Write-Host "Pi IP: $IP | Refresh: ${Interval}s | Updates: $Counter" -ForegroundColor Yellow
            Write-Host ""
            
            # Get live system metrics
            $SystemInfo = ssh ricoj100@$IP "echo 'CPU_TEMP:' && vcgencmd measure_temp && echo 'MEMORY:' && free -h | grep Mem && echo 'TPU_USB:' && lsusb | grep 1a6e && echo 'UPTIME:' && uptime && echo 'STORAGE:' && df -h / | tail -1"
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host " CONNECTION LOST TO PI" -ForegroundColor Red
                Write-Log "Lost connection to Pi at $IP" "ERROR"
            }
            
            Write-Host ""
            Write-Host " Monitoring active... (Ctrl+C to exit)" -ForegroundColor Yellow
...
switch ($Action) {
    "Monitor" { 
        Show-MonitoringDashboard -IP $PiIP -Interval $RefreshInterval
    }
    "Status" { 
