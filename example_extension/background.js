...
// Uses polyfill for browser.* vs chrome.* compatibility
...
// Enhanced error handling for background script (Mozilla Policy Compliance)
function handleBackgroundError(error, context = 'unknown') {
  ...
  // Store error for debugging only with user consent
  if (api && api.storage) {
    storeErrorWithConsent(error, context);
  }
}
...
// Privacy consent management (Mozilla Policy Compliance)
async function checkPrivacyConsent() {
  try {
    const result = await new Promise((resolve) => {
      api.storage.sync.get(['eq12_consent_given', 'eq12_privacy_consent'], resolve);
    });
    
    if (!result.eq12_consent_given) {
      // Show privacy consent page on first run
      api.tabs.create({
        url: api.runtime.getURL('privacy-consent.html'),
        active: true
      });
      return false;
    }
    
    return result.eq12_privacy_consent || {
      technical_data: false,
      usage_statistics: false,
      debug_info: false
    };
  } catch (error) {
    handleBackgroundError(error, 'privacy_consent_check');
    return false;
  }
}

// Data collection helper (Mozilla Policy Compliance)
async function canCollectData(dataType) {
  const consent = await checkPrivacyConsent();
  if (!consent) return false;
  
  switch (dataType) {
    case 'technical':
      return consent.technical_data;
    case 'usage':
      return consent.usage_statistics;
    case 'debug':
      return consent.debug_info;
    default:
      return false;
  }
}

// Enhanced error storage with consent checking
async function storeErrorWithConsent(error, context) {
  const canStore = await canCollectData('technical');
  if (!canStore) {
    EQ12Debug.logger.info('Background', 'Error not stored - no consent for technical data');
    return;
  }
  
  // Store error as before but only with consent
...
// Handle extension installation with enhanced debugging and privacy compliance
api.runtime.onInstalled.addListener(async (details) => {
  try {
    ...
    // Check privacy consent first (Mozilla Policy Compliance)
    ...
    const defaultConfig = {
      ...
      privacy_compliant: !!consentStatus
    };
...
// Initialize security and privacy components
let extensionComponents = null;

async function initializeSecurityComponents() {
  try {
    // Initialize privacy consent
    const privacyConsent = new PrivacyConsent();
    
    // Initialize security if components are available
    let security = null;
    if (typeof initializeSecurity !== 'undefined') {
      security = await initializeSecurity(privacyConsent);
    }
    
    return { privacyConsent, security };
  } catch (error) {
    handleBackgroundError(error, 'security_initialization');
    return { privacyConsent: null, security: null };
  }
}

// Initialize security components asynchronously
initializeSecurityComponents().then((components) => {
  extensionComponents = components;
  EQ12Debug.logger.info('Background', 'Security components initialized', components);
});
