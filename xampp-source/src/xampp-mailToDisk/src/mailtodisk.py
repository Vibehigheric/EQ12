...
class MailToDisk:
    ...
    dt = datetime.now()
    filename = "mail-{}-{}.txt".format(
        strftime("%Y%m%d-%H%M", gmtime()),
        dt.microsecond,
    )  # filename with date + time + milliseconds
    filename = f"{maildirectory}\{filename}"
    ...
    if os.path.exists(maildirectory):
        ...
        if folder.Size > 314572800:  # 300 MB
            warnfile = "{}\{}".format(
                maildirectory,
                "MAILTODISK_WRITE_RESTRICTION_FOLDER_MORE_THEN_300_MB.txt",
            )
            f = open(warnfile, "w")
            f.write(
                "MailtoDisk will NOT write in folder with a overall size of 300 MB (security limit). Please clean up this folder."
            )
            f.close()
            sys.exit(1)
