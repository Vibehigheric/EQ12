...
function Test-Prerequisites {
    ...
    $prerequisites = @{
        "Python"           = $false
        "PowerShell"       = $false
        "EQ12_Workspace"   = $false
        "Required_Scripts" = $false
    }
    ...
    if (Test-Path $WorkspacePath) {
        $prerequisites["EQ12_Workspace"] = $true
    ...
    if ($scriptsFound -eq $requiredScripts.Count) {
        $prerequisites["Required_Scripts"] = $true
    }
    ...
    if ($allMet) {
        ...
        Write-Log " Some prerequisites not met" "ERROR"
        $prerequisites.GetEnumerator() | Where-Object { -not $_.Value } | ForEach-Object {
            Write-Log "   Missing: $($_.Key)" "ERROR"
        }
    }
...
function Deploy-MonitoringSystem {
    ...
    try {
        ...
        $configPath = Join-Path $WorkspacePath "configs\resource_monitor_config.json"
        $monitoringConfig = @{
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            foreach ($line in $statusLines) {
                if ($line -match "Health Score|CPU|Memory|Alerts") {
                    Write-Log "   $line" "INFO"
                }
            }
...
function Start-MonitoringSystem {
    ...
    try {
        ...
        $job = Start-Job -ScriptBlock {
            ...
            & python @args
        } -ArgumentList "python", $arguments -Name "EQ12_ResourceMonitor"
...
function Stop-MonitoringSystem {
    ...
    try {
        ...
        # Stop any Python processes running the monitor
        $pythonProcesses = Get-Process -Name "python" -ErrorAction SilentlyContinue | 
        Where-Object { $_.CommandLine -like "*resource_monitor_wrapper*" }
        
        if ($pythonProcesses) {
            $pythonProcesses | Stop-Process -Force
            Write-Log " Python monitor processes stopped" "SUCCESS"
        }
...
function Get-MonitoringStatus {
    ...
    if ($pythonProcesses) {
        Write-Log " Python monitor process(es) found:" "SUCCESS"
        $pythonProcesses | ForEach-Object {
            Write-Log "   PID: $($_.Id), CPU: $($_.CPU)" "INFO"
        }
    }
    else {
        Write-Log " No Python monitor processes found" "INFO"
    }
    
    # Check recent logs
    $recentLogs = Get-ChildItem -Path $LogsDir -Filter "*resource_monitor*" | 
...
function Generate-MonitoringReport {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $reportFiles = Get-ChildItem -Path $LogsDir -Filter "*health_report*" | 
